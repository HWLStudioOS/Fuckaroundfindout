...
- The active health environment is pinned in `agents/health-requirements.txt`. Pillow 12.3.0 and pypdf 6.14.2 are installed, `pip check` passes and OSV reports no known vulnerabilities. Hosted ingestion requires a complete token, exact HTTPS URL and exact hostname allowlist.
