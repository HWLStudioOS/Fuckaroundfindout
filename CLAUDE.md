...
- One writer per state file. `agents/STATE-OWNERSHIP.md` declares who owns what, because on 13 August two automated passes and a review disagreed about who writes `today.md`. Ownership is declared, never inferred from prose. `today.md` must never carry a `linear:HWL-` marker; `this-week.md` owns all of them, and `scripts/check-state-consistency.mjs` fails if one appears in two files.
