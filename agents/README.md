...
| weekly-review | `agents/weekly-review.md` | Sun 18:00 | RUNNING-DEGRADED. Missed the scheduled 12 Jul run entirely: no `_log.md` entry, no `_review-log.md` entry, `this-week.md` never rewritten for the W26→W27 transition (partial inbox processing happened that day under no formal logged run). Recovered 19 Jul with a combined two-week catch-up run covering W26-W27. Same single-point-of-failure pattern as the 17-18 May miss. | Extended-run streak broken. Was 4/4 clean (7 Jun, 14 Jun, 28 Jun, 5 Jul) + ACCEPTANCE HIT (2/2 achieved 14 June), now 1 missed run since. Acceptance already banked, but reliability needs a direct look: confirm why launchd/mac mini didn't fire 12 Jul before counting on unattended Sundays. |
| weekly-cfo | `agents/weekly-cfo.md` | Fri 16:00 | RUNNING-DEGRADED (Xero still unreachable, manual snapshot used). Fifth run 17 July, same pattern, snapshot completed and flagged the Amex missed-payment escalation clearly. | 5/5 runs on the degraded-but-acceptable pattern (5 Jun, 12 Jun, 26 Jun, 3 Jul, 17 Jul). ACCEPTANCE HIT (2/2 achieved 12 June). Now on extended run, no gaps. |

**v1 ships when all three hit acceptance simultaneously.**

> As of 2026-07-19: weekly-cfo is the cleanest of the three (5 runs, no gaps, acceptance held). weekly-review hit acceptance 14 June but just broke its extended-run streak with a fully missed 12 Jul run, worth diagnosing before trusting it unattended again. morning-brief remains the sole blocker on its own acceptance bar: Strava has stopped being the problem, but verifier corrections are inconsistent week to week, so the 14-consecutive-clean bar still hasn't been hit. v1 is close on paper but the "3 agents unattended for 2 weeks" criterion needs weekly-review's gap explained, not just noted.
