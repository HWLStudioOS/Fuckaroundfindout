...
| `agents/_fleet-status.md` | generated by `scripts/fleet-health.py` | none | Rewritten in full on every run, twice daily. Never hand-edited, because the next run discards it. The script uses no Claude and no MCP on purpose: it exists to catch the shared Claude session logging out, and a watchdog that needs the thing it watches is not a watchdog. |
