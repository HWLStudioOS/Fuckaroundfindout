...
- `scripts/doctor.py` currently reports 15 passed, 2 warnings and 0 failures. The warnings are the known Claude logout and 56 deliberately preserved working paths.
...
- The active health environment is pinned in `agents/health-requirements.txt`. The pypdf pin moved to 6.15.0 on 9 August after CVE-2026-71870 and CVE-2026-71852 (crafted-PDF memory and runtime DoS, both fixed in 6.15.0) turned the health dependency audit red from 8 August. The Mac mini still has pypdf 6.14.2 installed until `pip install -r agents/health-requirements.txt` is rerun there. Pillow 12.3.0 remains clean per the same audit. Hosted ingestion requires a complete token, exact HTTPS URL and exact hostname allowlist.
...
- The nightly Board Room deploy was blocked on 4 and 5 August because deploy-side validation re-ran the board generator, which restamped `generatedAt` and `sourceCommit` in the tracked snapshot and always dirtied the pushed checkout. Fixed on 6 August: deploy validation now runs the generator in verify-only frozen-snapshot mode. The block was the mutation guard working as designed, not a bad deploy. The 6 to 8 August nightly deploys were then blocked by newly published high transitive advisories, js-yaml GHSA-5p4m-2wfm-xmqj and nanoid GHSA-2v37-7h3g-55p8, which also turned the Board Room CI job on `main` red. Both were patched on 9 August by override bumps to js-yaml 4.3.1 and nanoid 3.3.17 on branch `claude/sunday-evening-checkin-hdykdc` with audit, lint, all 16 tests and the production build passing. The fix deploys with the first nightly backup after that PR merges. The live site remained safe behind auth throughout, serving the 3 August deploy.
